...
module.exports.execute = async (client, message, args) => {
    ...
    let taglılar = message.guild.members.cache.filter(s => s.user.username.includes(tag,tag2) && !s.roles.cache.has(tagrol))
    let tagsızlar = message.guild.members.cache.filter(s => !s.user.username.includes(tag,tag2) && s.roles.cache.has(tagrol))
